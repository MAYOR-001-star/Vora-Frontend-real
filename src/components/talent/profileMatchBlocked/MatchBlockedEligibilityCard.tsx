      ...
      <div className="p-5 sm:p-7">
        ...
        <div className="bg-[#F7F7F7] border border-[#E6E6E6] rounded-lg p-4 sm:px-5 mb-[18px]">
          ...
          {reasons.map((reason, index) => (
            <div key={index} className="flex flex-col sm:flex-row sm:justify-between sm:items-start py-2 border-b border-[#E6E6E6] text-[13px] last:border-b-0 gap-1 sm:gap-4">
              <span className="text-gray-600 font-medium shrink-0">{reason.key}</span>
              <span className={`font-medium sm:text-right ${reason.key === 'Result' ? 'text-[#DC2626] text-[14px] font-bold' : 'text-[#1A1A1A]'}`}>
                {reason.value}
              </span>
            </div>
          ))}
